...
Starting with v0.6.6, installed Mac copies use a persistent **per-Mac explicit designated requirement (DR)**. No private signing key or certificate exists. A random local marker is stored under the user's Application Support directory, injected into the staged app's `Info.plist`, and included in the explicit DR together with the fixed bundle identifier. Before an update replaces the installed app, the verified release artifact is staged and re-signed ad-hoc with that same DR.
